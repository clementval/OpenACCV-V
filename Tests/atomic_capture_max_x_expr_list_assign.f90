RECURSIVE FUNCTION IS_POSSIBLE(a, b, c, length, init) RESULT(POSSIBLE)
...
DO x = 1, length
  IF (abs(c(x) - max(init, a(x), b(x))) .lt. ((10 - length) * PRECISION)) THEN
    DO y = 1, x - 1
      passed_a(y) = a(y)
      passed_b(y) = b(y)
    END DO
...
!$acc data copyin(a(1:LOOPCOUNT, 1:10), b(1:LOOPCOUNT, 1:10)) copy(totals(1:LOOPCOUNT)) copyout(c(1:LOOPCOUNT, 1:10))
  !$acc parallel
    ...
    DO x = 1, LOOPCOUNT
      DO y = 1, 10
        !$acc atomic capture
          totals(x) = max(totals(x), a(x, y), b(x, y))
          c(x, y) = totals(x)
        !$acc end atomic
      END DO
    END DO
